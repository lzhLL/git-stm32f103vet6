...
}GPIOMode_TypeDef;		//GPIO工作模式


typedef struct
{
	uint16_t GPIO_Pin;		//GPIO的引脚
	uint16_t GPIO_Speed;	//GPIO工作速度
	uint16_t GPIO_Mode;		//GPIO工作模式
}GPIO_InitTypeDef;




void GPIO_SetBits(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin);		//相当于将ODR输出寄存器置1
void GPIO_ResetBits(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin);	//相当于将ODR输出寄存器置0
